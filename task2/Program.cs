...
// // Console.WriteLine(array[0]); Каким образом можно обратиться к массиву и получить
// //                              значение соответствующего элемента по указаному индексу
// int result = Max(
...
while (index < n)
{
    if(array[index] == find) // == знак если совпадения/равенства
    {
        Console.WriteLine(index);
        break;  //Прерывание алгоритма, чтобы показывался 
                //первый номер массива с нужным числом, 
                //если в масиве имеются несколько одинаковых чисел
    }
    //index = index + 1;
    index++;
}